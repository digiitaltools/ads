(function() {
    // 1. FITUR AUTO-UPGRADE KE HTTPS
    // Jika halaman diakses via http, otomatis redirect ke https
    if (window.location.protocol === 'http:') {
        window.location.href = window.location.href.replace('http:', 'https:');
        return; // Hentikan eksekusi script sampai halaman reload ke https
    }

    // 2. Jika sudah di HTTPS, jalankan logika iklan
    const secureProto = 'https:';
    
    // Injeksi CSP Upgrade-Insecure-Requests untuk keamanan tambahan
    try {
    ...
    // 3. URL Iklan (Dipaksa HTTPS)
    const secureSrc = secureProto + "//www.highperformanceformat.com/c6519a79b77606d968cf36c00f3894c6/invoke.js";
